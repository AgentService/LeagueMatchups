...
const downloadFolder = 'champion_images'; // Name des Zielordners
...
async function downloadAndSaveChampionImages(championsData, version) {
  ...
  for (const championKey in championsData) {
    ...
    const imageUrl = `${riotApiUrl}/cdn/${version}/img/champion/${champion.image.full}`;
    const imagePath = path.join(downloadFolder, champion.image.full); // Pfad für das gespeicherte Bild
