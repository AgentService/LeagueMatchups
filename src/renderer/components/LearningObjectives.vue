        ...
        <div class="lo-cards-container">
          ...
          <div v-for="lo in categorizedObjectives[category]" :key="lo.name" class="lo-card">
            <div class="title-container">
              <div class="lo-title">
                {{ lo.name }}
                <i v-if="lo.completed" class="fas fa-check-circle text-success"></i>
              </div>
            </div>
...
.lo-category-section::before {
  ...
  position: absolute;
  left: 0;
  height: 90%; /* Set to 90% of the element’s height */
  width: 3px; /* Width of the border */
  background: linear-gradient(to top, #000000, #0D2029, #000000); /* Gradient from black to base color and back to black */
  top: 5%; /* Centering the border vertically */
}
...
.lo-cards-container {
  ...
  /* Enable flexbox */
  flex-direction: row;
  /* Align LOs horizontally */
  overflow-x: auto;
  /* Enable horizontal scrolling */
  align-items: center;
  /* Vertically center the LOs */
  gap: 10px;
...
.banner-progress-bar {
  background: var(--red-laser-gradient);
...
.lo-cards-container::-webkit-scrollbar {
  height: 12px; /* Height of the scrollbar */
  width: 12px; /* Width of the scrollbar */
}

.lo-cards-container::-webkit-scrollbar-thumb {
  background-color: #003366; /* Dark blue color for the thumb (scrolling part) */
  border-radius: 6px; /* Rounded corners for the thumb */
}

.lo-cards-container::-webkit-scrollbar-track {
  background-color: #000000; /* Black color for the track (background part) */
}

/* Style for Firefox */
.lo-cards-container {
  padding: 0.5rem;
  scrollbar-color: #005cb8 #000000; /* Dark blue thumb on a black track */
