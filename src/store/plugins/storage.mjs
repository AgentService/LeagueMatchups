...
export function getTokenFromLocalStorage(key) {
  try {
    ...
    console.error(
      `Error retrieving token from localStorage for key: ${key}`,
      error
    );
    return null;
  }
...
export function removeFromSessionStorage(key) {
  try {
    ...
    console.error(
      `Error removing data from sessionStorage for key: ${key}`,
      error
    );
  }
}
...
export function saveTokenToLocalStorage(key, token) {
  try {
    localStorage.setItem(key, token);
  } catch (error) {
    console.error(`Error saving token to localStorage for key: ${key}`, error);
  }
}
