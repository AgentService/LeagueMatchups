...
class Body {
  ...
  async formData() {
    ...
    const { toFormData } = await Promise.resolve().then(() => require("./multipart-parser-SRkn2huJ.js"));
...
const Debug$1 = /* @__PURE__ */ getDefaultExportFromCjs(srcExports);
Debug$1("app:champ-select-session");
Debug$1.enable("*");
class ChampSelectSession {
  ...
  findOwnActionId(actionType) {
    for (let actionGroup of this.actions) {
      for (let action of actionGroup) {
        if (action.actorCellId === this.localPlayerCellId && action.type === actionType) {
          return action.id;
        }
      }
    }
  ...
  isBanPhase() {
    return this.actions.flat().some((action) => action.type === "BAN" && !(action == null ? void 0 : action.completed));
  }
  isPickPhase() {
    return this.actions.flat().some((action) => action.type === "PICK" && !(action == null ? void 0 : action.completed));
  }
  isEnemyTurn() {
    return this.actions.flat().some(
      (action) => action.isInProgress && !action.isAllyAction && action.actorCellId !== this.localPlayerCellId
    );
  }
  isLocalPlayerTurn() {
    return this.actions.flat().some(
      (action) => action.actorCellId === this.localPlayerCellId && action.isInProgress
    );
  }
  isLocalPlayerTurnCompleted() {
    return this.actions.flat().some(
      (action) => action.actorCellId === this.localPlayerCellId && !action.isInProgress && action.completed
    );
  }
  getLockedInChampions(isAlly = true) {
    const team = isAlly ? this.myTeam : this.theirTeam;
    return team.filter((member) => member.championId > 0).map((member) => member.championId);
  }
  // Add other methods as needed based on the functionalities you wish to implement.
}
const { authenticate: authenticate$1 } = require("league-connect");
const debug$1 = Debug$1("app:websocket-events");
Debug$1.enable("*");
class WebSocketEventHandlers extends EventEmitter$1 {
  ...
  setup(ws) {
    debug$1("Setting up WebSocket event handlers.");
    ws.subscribe(
    ...
    ws.subscribe(
      ...
      (event) => this.handleGameflowPhaseEvent(event)
    );
  }
  handleEvent(event) {
  ...
  handleCreate(event) {
    debug$1("Champ select session created.");
    this.champSelectSession = new ChampSelectSession(event);
  }
  handleDelete() {
    debug$1("Champ select session deleted.");
    this.champSelectSession = null;
  ...
  handleGameflowPhaseEvent(event) {
    debug$1("Gameflow phase event received:", event);
  ...
  async fetchPostGameData() {
    try {
      ...
      debug$1("Fetching match history from endpoint:", endpoint);
      ...
      if (matchHistoryData && matchHistoryData.games && matchHistoryData.games.games.length > 0) {
        for (let i2 = 0; i2 < Math.min(10, matchHistoryData.games.games.length); i2++) {
          const game = matchHistoryData.games.games[i2];
        }
        const validGames = matchHistoryData.games.games.filter(
          (game) => game.endOfGameResult !== "Abort_TooFewPlayers" && game.gameMode === "CLASSIC" && game.queueId === 420
          // Check for ranked queues
        );
        if (validGames.length > 0) {
          const mostRecentGame = validGames[0];
          this.sendToMainWindow("post-game-stats", mostRecentGame);
          debug$1("Sent post-game stats to main window for game:", mostRecentGame.gameId);
        } else {
          console.log("No valid completed ranked games found.");
          debug$1("No valid ranked games found after filtering.");
        }
      } else {
        console.log("No game data available.");
        debug$1("Match history data is empty or does not contain any games.");
      }
    } catch (error) {
      console.error("Error fetching post-game stats:", error);
      debug$1("Error occurred during fetchPostGameData execution:", error);
    }
  }
  async fetchFromApi(endpoint) {
    const credentials = await authenticate$1({ awaitConnection: false });
  ...
  reflectChanges(oldSessionData, newSessionData) {
    if (!oldSessionData.isLocalPlayerTurn() && newSessionData.isLocalPlayerTurn()) {
      console.log("It's now the local player's turn.");
    }
    if (newSessionData.isLocalPlayerTurn() || newSessionData.isLocalPlayerTurnCompleted()) {
    ...
    if (newlyLockedInChampions.length > 0) {
      debug$1("Newly locked in champions:", newlyLockedInChampions);
      debug$1("My team:", newSessionData.myTeam);
      debug$1("Their team:", newSessionData.theirTeam);
  ...
  handlePlayerPickChanges(oldSessionData, newSessionData) {
    ...
    if (newLocalPlayerPickAction) {
      const isChampionChange = this.previousPickState.championId !== newLocalPlayerPickAction.championId;
      const isCompletionChange = this.previousPickState.completed !== newLocalPlayerPickAction.completed;
...
class MockAutoUpdater extends EventEmitter {
  ...
  checkForUpdatesAndNotify() {
    ...
    return Promise.resolve({
      ...
      downloadPromise: this.autoDownload ? Promise.resolve(["path/to/download"]) : void 0,
      // cancellationToken not needed per your requirement
      versionInfo: this.mockUpdateInfo
      // Assuming for backward compatibility
    });
  }
...
new MockAutoUpdater();
let updater = process.env.NODE_ENV === "DEVELOPMENT" ? autoUpdater : autoUpdater;
...
store.set("detectionMethod", store.get("detectionMethod", "process"));
async function initializeWebSocket(credentials) {
  try {
    ...
    if (credentials) {
      ...
      const webSocketEventHandlers = new WebSocketEventHandlers(exports.mainWindow);
...
async function fetchSummonerNameWithRetry(credentials, retries = 3, interval = 5e3, initialDelay = 2e3) {
  if (initialDelay > 0) {
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      return await fetchSummonerName(credentials);
    } catch (error) {
...
async function setupLeagueClientMonitoring() {
  try {
    ...
    const client = new LeagueClient(credentials, { pollInterval: 1e3 });
    ...
    client.on("connect", async (newCredentials) => {
      ...
      global.credentials = newCredentials;
...
async function fetchAndSendSummonerName(credentials) {
  try {
    currentSummonerResponse = await fetchSummonerNameWithRetry(credentials);
    if (exports.mainWindow && exports.mainWindow.webContents) {
      exports.mainWindow.webContents.send("summoner-name-response", currentSummonerResponse);
    }
  } catch (error) {
...
function notifyClientStatus(isConnected) {
  if (exports.mainWindow && exports.mainWindow.webContents) {
    exports.mainWindow.webContents.send("client-status", { connected: isConnected });
  }
}
...
ipcMain.on("get-summoner-name", async (event) => {
  if (currentSummonerResponse) {
    ...
    event.reply("summoner-name-response", currentSummonerResponse);
  } else {
    console.log("No summoner data available, fetching new data...");
    const newCredentials = global.credentials;
    if (newCredentials) {
...
async function fetchSummonerName(credentials) {
  ...
  try {
    ...
    console.log("Summoner and region data fetched:", combinedResponse);
...
function createMainWindow() {
  ...
  exports.mainWindow = new BrowserWindow({
    ...
    y,
    minWidth: 1850,
    // set the minimum width 1600
    minHeight: 900,
    // set the minimum height 800
    width: 2560,
    height: 1240,
    // maxHeight: 800,
    // maxWidth: 1280,
    ...
    webPreferences: {
      ...
      // Keep it true for security
      nodeIntegration: false
      // Node integration should be false in renderer for security
    }
  });
  ...
  if (MAIN_WINDOW_VITE_DEV_SERVER_URL) {
    exports.mainWindow.loadURL(MAIN_WINDOW_VITE_DEV_SERVER_URL);
  } else {
    log.info(
      "Loading main window from file",
      path.join(__dirname, `../renderer/${MAIN_WINDOW_VITE_NAME}/index.html`)
    );
    exports.mainWindow.loadFile(
      path.join(__dirname, `../renderer/${MAIN_WINDOW_VITE_NAME}/index.html`)
    );
  }
...
updater.on("download-progress", (progressObj) => {
  ...
  if (mainWindowReady && exports.mainWindow && exports.mainWindow.webContents) {
    exports.mainWindow.webContents.send("download-progress", progressObj);
  }
});
updater.on("update-available", (info) => {
  log.info("Update available.", info);
  if (mainWindowReady && exports.mainWindow && exports.mainWindow.webContents) {
    exports.mainWindow.webContents.send("update-available", info);
  }
});
updater.on("update-error", (error) => {
  if (mainWindowReady && exports.mainWindow && exports.mainWindow.webContents) {
    exports.mainWindow.webContents.send("update-error", error);
  }
  dialog.showErrorBox(
    "Error: ",
  ...
  updater.on("update-downloaded", (info) => {
    if (isAppStartup) {
      ...
      if (exports.mainWindow && !exports.mainWindow.isDestroyed()) {
        exports.mainWindow.webContents.send("update-downloaded", info);
      }
    }
  });
